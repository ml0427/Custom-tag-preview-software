...
const coverUrl = computed(() => {
    return props.comic ? api.getCoverUrl(props.comic.id) : '';
});
...
watch(() => props.comic, (newComic) => {
    zipImages.value = [];
    if (newComic) {
        // We defer loading images until user clicks "Preview Images", or load immediately?
        // Let's load immediately for wow factor
        loadImages();
    }
});

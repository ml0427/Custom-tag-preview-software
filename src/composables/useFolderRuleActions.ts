...
export function useFolderRuleActions(
  ...
  const applyRulesForTarget = async (target: RuleTarget) => {
    ...
    const explicitType = target.presetTypeId != null
      ? types.find(t => t.id === target.presetTypeId)
      : undefined;

    const rules = explicitType?.tagRules ?? await resolveEffectiveItemRules(dbItem, types, target.category);
    ...
    try {
      ...
      const result = await api.applyRulesToItem(dbItem.id, rules);
